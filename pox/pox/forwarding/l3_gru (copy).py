...
sudo mn --switch ovsk --topo tree,depth=2,fanout=2 --controller=remote,ip=127.0.0.1,port=6633
xterm h1 h4

hping3 10.0.0.4 -c 10000 --flood --rand-source -w 64 -p 0 -S -d 120

hping3 -S 10.0.0.4 -p 0
...
lib_path = os.path.abspath(os.path.join('/home/tannc/kltn/gru'))
...
class l3_switch(EventMixin):
    ...
    def _handle_openflow_PacketIn(self, event):
        ...
        if isinstance(packet.next, ipv4):
            ...
            x_test = [len(packet), packet.next.iplen, packet.next.srcip, packet.next.desip, packet.next.next.srcport, packet.next.next.dstport, packet.next.protocol]
            defender = Detector()
            packets.append(x_test)
            dpids.append(event.connection.dpid)
            if len(packets) >= 25:
                detectionValue = defender.predictResult(packets)
                print("value: ", normal)
                with open('/home/tannc/kltn/thres.txt', 'a') as f:
                    f.write(str(detectionValue) + "\n")
                if detectionValue[0][0]>0.5:
                    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~normal~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
                    print('the prediction from GRU DL model: ', detectionValue[0][0], '(1: normal, 0: attack)')
                    packets.pop(0)
                    dpids.pop(0)
                else:
